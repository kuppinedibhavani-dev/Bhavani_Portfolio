...
const Navbar = ({ darkMode, toggleDarkMode }) => {
  ...
  return (
    <div className="flex justify-center w-full-fixed z-50 px-8 mt-4">
      ...
      className={` flex items-center justify-center ${colors.navBg} backdrop-blur-lg rounded-2xl px-4 bg:px-8 py-2 shadow-lg`}>
      <div className="flex items-center justify-between w-full space-x-6 lg:space-x-8 ">
        ...
        className='text-xl font-bold'>
          ...
          {/*Menu*/}
          <ul className="flex gap-8 lg:hidden">
            {navItems.map((item)=>(
              <li key={item.name}>
                <a
                href={item.link}
  ...
  darkMode ? "bg-gray-700" : "bg-gray-200"
        ...
        arial-label={darkMode ? "Switch to Light Mode" 
        : "Switch to Dark Mode"}>
            {darkMode ? (
